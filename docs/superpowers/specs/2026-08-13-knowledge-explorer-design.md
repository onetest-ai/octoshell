...
- `add-workflow.js` and `validate.js`, **each in two copies** (`resources/octobots-pack/…` and the
  installed `.claude/…`);
- `workflow-designer/SKILL.md` and `mission-planner/SKILL.md`, likewise doubled;
- `packages/board/src/validate.ts`, for validator parity;
- the tests that pin the one-per-mission rule.
...
2. **The `.claude/` twins are reported as invisible to the instrument, not as uncoupled.** A run
   that silently drops four real files while reading as confident is a failure even if condition 1
   passes.
