...
- [`agent-generated-system-load-must-be-torn-down-by-a-trap.md`](agent-generated-system-load-must-be-torn-down-by-a-trap.md) — an agent's CPU-contention experiment leaked 20 busy loops and 13.7 cores for 70 minutes; recorded PIDs are not a teardown mechanism, a `trap` is.
- [`graph-drift-excludepaths-applies-only-in-drift-not-harvest.md`](graph-drift-excludepaths-applies-only-in-drift-not-harvest.md) — `octograph.yaml`'s `excludePaths` (default `.agents/`/`.claude/`/`.octobots/`) is read only inside `drift()`; `harvest`/`analyze`/`impact`/clustering still see those paths on purpose, so `impact` can still answer "what else should I update" for an agent's own notes.
