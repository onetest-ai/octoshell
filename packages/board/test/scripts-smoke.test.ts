...
describe("migrate script — md → yaml entity migration", () => {
  it("migrates a legacy markdown board to yaml, folding parent markers and tokenomics", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "- [ ] works",
        "",
        "<!-- Auto-generated by Octobots from the mission's fields above. -->",
        "",
        "## Tasks",
    ...
    const out = runScript("migrate.js", [], projectDir); // root defaults to `.octobots` under cwd
    expect(out.trim()).toContain("migrated 3 entity file(s) to yaml");

    // Every .md is trashed; every .yaml exists.
    expect(existsSync(join(tDir, "task.md"))).toBe(false);
    expect(existsSync(join(tDir, "task.yaml"))).toBe(true);
    expect(readFileSync(join(tDir, "task.yaml"), "utf8")).toContain("role: python-dev");
    ...
    expect(m.status).toBe("executing"); // active → executing, folded from the campaign's ## Missions line
