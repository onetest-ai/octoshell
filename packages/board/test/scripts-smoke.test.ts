...
describe("add-doc script — creates the ## Documents section on demand", () => {
  it("attaches a link to a freshly-scripted campaign that has no ## Documents section yet", () => {
    runScript("add-campaign.js", ["Camp"], projectDir);
    const campaignMd = join(boardRoot, "campaigns", "camp", "campaign.md");
    // add-campaign deliberately does NOT scaffold a Documents section (parity with createCampaign).
    expect(readFileSync(campaignMd, "utf8")).not.toContain("## Documents");

    runScript("add-doc.js", [campaignMd, "M1 spec", "docs/spec.md"], projectDir);

    const text = readFileSync(campaignMd, "utf8");
    expect(text).toContain("## Documents");
    expect(text).toContain("- [M1 spec](docs/spec.md)");
    // The section landed in the managed area, above the boundary comment, so links round-trip.
    expect(text.indexOf("## Documents")).toBeLessThan(text.indexOf("Auto-generated by Octobots"));
    expect(runScript("validate.js", [campaignMd], projectDir)).toContain("OK");
  });
});
