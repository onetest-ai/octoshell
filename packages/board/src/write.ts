...
/**
 * Read a `[key:value]` marker from a board line identified by entity name within a section.
 * Returns the value string, or null if the line or marker is not found.
 */
function readLineMarker(
  parentMd: string,
  section: "## Missions" | "## Tasks" | "## Bugs",
  entityName: string,
  key: string,
): string | null {
  if (!existsSync(parentMd)) return null;
  const text = readFileSync(parentMd, "utf8");
  const lines = text.split("\n");
  const headIdx = lines.findIndex((l) => new RegExp(`^${section}\\s*$`).test(l));
  ...
  for (let i = headIdx + 1; i < lines.length; i++) {
    ...
    const { markers, bare } = splitMarkers(bullet[2] ?? "");
    if (boardLineEntityName(bare).toLowerCase() !== nameKey) continue;
    return markers.find((m) => m.key === key)?.value ?? null;
  }
  return null;
}
...
/**
 * Upsert (or clear when slug is null) a `[key:value]` marker stored as the FIRST LINE of the
 * preserved tail — immediately after the `<!-- Auto-generated by Octobots` boundary comment.
 * This position survives `updateBrief`, which preserves everything below the boundary verbatim.
 *
 * If no boundary comment exists, we append one followed by the marker so the file gains a valid
 * boundary for future updateBrief calls.
 */
function upsertFileTailMarker(mdPath: string, key: string, value: string | null): boolean {
  if (!existsSync(mdPath)) return false;
  const text = readFileSync(mdPath, "utf8");
  const boundaryRe = /^<!-- Auto-generated by Octobots[^\n]*-->/m;
  const markerRe = new RegExp(`^\\[${key}:[^\\]]*\\]\\s*$`);
  const boundaryIdx = text.search(boundaryRe);

  if (boundaryIdx < 0) {
    // No boundary yet — if clearing, nothing to do; if setting, append boundary + marker.
    if (value === null) return true;
    const markerLine = `[${key}:${value}]`;
    const sep = text.endsWith("\n") ? "" : "\n";
    const boundary = `<!-- Auto-generated by Octobots from the campaign's fields above. The orchestrator owns any sections appended below this comment. -->`;
    writeFileSync(mdPath, `${text}${sep}${boundary}\n${markerLine}\n`, "utf8");
    return true;
  }

  // Locate the end of the boundary line (the \n after -->).
  const boundaryEnd = text.indexOf("\n", boundaryIdx);
  const afterBoundary = boundaryEnd >= 0 ? boundaryIdx + (boundaryEnd - boundaryIdx) + 1 : text.length;
  const tail = boundaryEnd >= 0 ? text.slice(afterBoundary) : "";
  const head = text.slice(0, afterBoundary);

  // Check if the very first content line of the tail is already a [key:...] marker.
  const tailLines = tail.split("\n");
  const existingIdx = tailLines.findIndex((l) => markerRe.test(l.trim()));

  if (value === null) {
    // Remove the marker line if present.
    if (existingIdx < 0) return true; // already absent — idempotent success
    tailLines.splice(existingIdx, 1);
  } else {
    const markerLine = `[${key}:${value}]`;
    if (existingIdx >= 0) {
      tailLines[existingIdx] = markerLine;
    } else {
      // Insert as the first line of the tail (index 0).
      tailLines.splice(0, 0, markerLine);
    }
  }

  writeFileSync(mdPath, head + tailLines.join("\n"), "utf8");
  return true;
}

/**
 * Read a `[key:<value>]` marker from the first lines of the preserved tail
 * (immediately after the `<!-- Auto-generated by Octobots` boundary comment).
 * Returns the value string, or null if absent.
 */
function readFileTailMarker(mdPath: string, key: string): string | null {
  ...
  const tail = text.slice(boundaryEnd + 1);
  const markerRe = new RegExp(`^\\[${key}:([^\\]]*)\\]\\s*$`);
...
/**
 * Set (slug) or clear (null) the `[team:<slug>]` marker on an entity's board location.
 *
 * - mission: the `[team:X]` marker on its line in parent campaign.md `## Missions`.
 * - campaign: a standalone `[team:X]` marker line at the top of campaign.md's preserved tail
 *   (immediately after the `<!-- Auto-generated by Octobots` boundary comment). This position
 *   survives `updateBrief`, which preserves the tail verbatim.
 *
 * Returns true on success, false when the entity is not found.
 */
export function setTeam(root: string, kind: "campaign" | "mission", id: string, slug: string | null): boolean {
  const board = new BoardModel(root);
  board.rebuild();
  if (kind === "mission") {
    const m = board.getMission(id);
    ...
    const c = board.getCampaign(m.campaignId);
    if (!c) return false;
    return upsertLineMarker(join(root, c.folderPath, "campaign.md"), "## Missions", m.title, "team", slug);
  }
  // campaign — store in the preserved tail, not the managed head
  const c = board.getCampaign(id);
  if (!c) return false;
  return upsertFileTailMarker(join(root, c.folderPath, "campaign.md"), "team", slug);
}

/**
 * Read the `[team:<slug>]` marker for an entity, or null if not set.
 *
 * - mission: reads the marker from its line in parent campaign.md `## Missions`.
 * - campaign: reads the standalone marker from the campaign's own campaign.md preserved tail
 *   (immediately after the `<!-- Auto-generated by Octobots` boundary comment).
 */
export function getTeam(root: string, kind: "campaign" | "mission", id: string): string | null {
  const board = new BoardModel(root);
  board.rebuild();
  if (kind === "mission") {
    const m = board.getMission(id);
    ...
    const c = board.getCampaign(m.campaignId);
    if (!c) return null;
    return readLineMarker(join(root, c.folderPath, "campaign.md"), "## Missions", m.title, "team");
  }
  // campaign — read from the preserved tail
  const c = board.getCampaign(id);
  if (!c) return null;
  return readFileTailMarker(join(root, c.folderPath, "campaign.md"), "team");
}
