...
function parseParentMarkers(text: string, section: "## Tasks" | "## Bugs" | "## Missions"): Map<string, ParentMarker> {
  ...
  // The section runs until the next SIBLING board heading (## Tasks/## Bugs/## Missions), NOT the next
  // heading of any kind — a `## Tokenomics` (or any other) block appended between the heading and the
  // board lines must not truncate the scan and orphan the `[status:]` markers below it. Bullet-line
  // matching keys each marker by entity name, so scanning past a content section is safe.
  const nextHeading = after.search(/^##\s+(?:Tasks|Bugs|Missions)\s*$/m);
 ...
 * own `##` headings (`## Decision`, `## Update`, `## Product decisions`, …). Everything below the
 * boundary that is NOT a recognised structural section — the board-line projections
 * (`## Tasks`/`## Bugs`/`## Missions`), the folded `## Tokenomics`, or a placeholder-only stub — is
 * preserved verbatim into the entity's `notes` field. Returns undefined when nothing meaningful is left.
 */
function parseNotes(md: string): string | undefined {
  const marker = md.indexOf("Auto-generated by Octobots");
  if (marker < 0) return undefined;
  const close = md.indexOf("-->", marker);
  if (close < 0) return undefined;
  const tail = md.slice(close + 3);
  // Structural sections are handled elsewhere — never fold them into notes.
  const STRUCTURAL = /^(Tasks|Bugs|Missions|Tokenomics|Sessions|Runs)\b/i;
  const kept: string[] = [];
  // Split the agent-owned tail into `## `-headed parts (any leading chunk before the first heading
  // is kept too). Drop structural and placeholder-only sections; keep the rest verbatim.
  for (const part of tail.split(/(?=^##[ \t]+)/m)) {
    const heading = /^##[ \t]+(.+?)[ \t]*$/m.exec(part);
    if (heading && STRUCTURAL.test(heading[1] ?? "")) continue;
    // Placeholder-only content (`_(none)_`, `_(no sub-tasks — atomic task)_`, bare whitespace) carries nothing.
